...
void VulkanRenderer::Initialize(Window& window)
{
    ...
    // TEMP //
    CreateMaterial("Shaders/vert.spv", "Shaders/frag.spv");
...
void VulkanRenderer::Cleanup()
{
    ...
    for (VkFramebuffer frameBuffer : _swapChainFrameBuffers) vkDestroyFramebuffer(_device, frameBuffer, nullptr);

    for (VulkanMaterial material : _materialList) material.Cleanup(_device);
    
    vkDestroyRenderPass(_device, _renderPass, nullptr);

    for (VkImageView imageView : _swapChainImageViews) vkDestroyImageView(_device, imageView, nullptr);
...
int VulkanRenderer::CreateMaterial(const char* vertexShader, const char* fragmentShader)
{
    int id;
    _materialList.push_back(VulkanMaterial(_device, _renderPass, vertexShader, fragmentShader, &id));

    return id;
}

void VulkanRenderer::CreateInstance(const char* title)
{
    ...
    if (vkCreateInstance(&createInfo, nullptr, &_instance) != VK_SUCCESS) throw std::runtime_error("failed to create instance!");
}
...
void VulkanRenderer::SetupDebugMessenger() 
{
    ...
    if (CreateDebugUtilsMessengerEXT(_instance, &createInfo, nullptr, &_debugMessenger) != VK_SUCCESS) throw std::runtime_error("failed to set up debug messenger!");
}
...
void VulkanRenderer::PickPhysicalDevice() 
{
    ...
    if (_physicalDevice == VK_NULL_HANDLE) throw std::runtime_error("failed to find a suitable GPU!");
}
...
void VulkanRenderer::CreateLogicalDevice() 
{
    ...
    if (vkCreateDevice(_physicalDevice, &createInfo, nullptr, &_device) != VK_SUCCESS) throw std::runtime_error("failed to create logical device!");
...
{
    if (glfwCreateWindowSurface(_instance, window, nullptr, &_surface) != VK_SUCCESS) throw std::runtime_error("failed to create window surface!");
}
...
void VulkanRenderer::CreateSwapChain(int width, int height) 
{
    ...
    if (vkCreateSwapchainKHR(_device, &createInfo, nullptr, &_swapChain) != VK_SUCCESS) throw std::runtime_error("failed to create swap chain!");
...
void VulkanRenderer::CreateImageViews() 
{
    ...
    for (size_t i = 0; i < _swapChainImages.size(); i++) {
        ...
        if (vkCreateImageView(_device, &createInfo, nullptr, &_swapChainImageViews[i]) != VK_SUCCESS) throw std::runtime_error("failed to create image views!");
    }
}
...
void VulkanRenderer::CreateRenderPass() 
{
    ...
    if (vkCreateRenderPass(_device, &renderPassInfo, nullptr, &_renderPass) != VK_SUCCESS) throw std::runtime_error("failed to create render pass!");
}
...
void VulkanRenderer::CreateFrameBuffers() 
{
    ...
    for (size_t i = 0; i < _swapChainImageViews.size(); i++) 
    {
        ...
        if (vkCreateFramebuffer(_device, &framebufferInfo, nullptr, &_swapChainFrameBuffers[i]) != VK_SUCCESS) throw std::runtime_error("failed to create framebuffer!");
    }
}
...
void VulkanRenderer::CreateCommandPool()
{
    ...
    if (vkCreateCommandPool(_device, &poolInfo, nullptr, &_commandPool) != VK_SUCCESS) throw std::runtime_error("failed to create command pool!");
}
...
void VulkanRenderer::DrawFrame(DrawData& drawData)
{
    ...
    if (vkQueueSubmit(_graphicsQueue, 1, &submitInfo, _inFlightFences[_currentFrame]) != VK_SUCCESS) throw std::runtime_error("failed to submit draw command buffer!");
...
void VulkanRenderer::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
    ...
    if (vkCreateBuffer(_device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) throw std::runtime_error("failed to create buffer!");
    ...
    if (vkAllocateMemory(_device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) throw std::runtime_error("failed to allocate buffer memory!");
